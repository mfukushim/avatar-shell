            ...
            <template v-slot:after>
              ...
              >
                <q-tab-panel name="general">
                  ...
                  <q-card>
                    <q-card-section>
                      <q-input v-model="editingSettings!.general.name" :label="$t('avatarTemplateName')" data-testid="as-general-name" />
                      <div class="row">
                        <div class="col-6 q-pa-sm">
                          <q-input type="number" v-model.number="editingSettings!.general.maxGeneratorUseCount" :label="$t('generatorUsageLimit')" data-testid="as-general-max-generator-use" />
                        </div>
                      </div>
                    </q-card-section>
                  </q-card>
                    ...
                    <q-tab-panel v-for="daemon in editingSchedulers" :key="daemon.id" :name="daemon.name">
                      <q-card>
                        <q-card-section class="row items-center">
                          <q-input bottom-slots class="" v-model="daemon.name" debounce="1000" @change="tabDaemon = daemon.name" :data-testid="`daemon-name-${daemon.id}`">
                            <template v-slot:hint>
                              {{ $t('name') }}
                            </template>
                          </q-input>
                          <q-toggle class="" v-model="daemon.isEnabled" :label="$t('enable')" :data-testid="`daemon-enabled-${daemon.id}`" />
                          <q-space/>
                          <q-btn icon="add" @click="deleteScheduler(daemon)" :data-testid="`daemon-delete-${daemon.id}`">{{ $t('delete') }}</q-btn>
                        </q-card-section>
                        <div class="row">
                        <div class="col-4">
                          <div class="q-pa-sm shadow-2 q-ma-md">
                            {{$t('conditions')}}
                          <q-select class="q-ma-sm" v-model="daemon.trigger.triggerType" options-dense map-options @update:model-value="daemon.trigger.triggerType = $event.value" :options="DaemonTriggerList.map(value => ({value,label:$t(`trigger.${value}`)}))" label="起動条件" :data-testid="`daemon-${daemon.id}-trigger-type`"/>
                          <div v-if="daemon.trigger.triggerType === 'Startup'">
                            {{$t('trigger.Startup')}}
                          </div>
                          <div v-else-if="daemon.trigger.triggerType === 'TalkAfterMin'">
                            {{$t('trigger.TalkAfterMin')}}
                            <q-input type="number" v-model.number="daemon.trigger.condition.min" :label="$t('min')" :data-testid="`daemon-${daemon.id}-talkafter-min`" />
                          </div>
                          <div v-else-if="daemon.trigger.triggerType === 'IfContextExists'">
                            ...
                            <div v-else-if="daemon.trigger.triggerType === 'IfSummaryCounterOver'">
                              ...
                              <q-input type="number" v-model.number="daemon.trigger.condition.countMax" :label="$t('convNum')" :data-testid="`daemon-${daemon.id}-summary-countmax`" />
                            </div>
                            <div v-else-if="daemon.trigger.triggerType === 'IfExtTalkCounterOver'">
                              {{$t('trigger.IfExtTalkCounterOver')}}
                              <q-input type="number" v-model.number="daemon.trigger.condition.countMax" :label="$t('convNum')" :data-testid="`daemon-${daemon.id}-exttalk-countmax`" />
                            </div>
                          <div v-else-if="daemon.trigger.triggerType === 'TimerMin'">
                            {{$t('trigger.TimerMin')}}
                            <q-input type="number" v-model.number="daemon.trigger.condition.min" :label="$t('min')" :data-testid="`daemon-${daemon.id}-timer-min`" />
                            <q-toggle v-model="daemon.trigger.condition.isRepeatMin" :label="$t('loop')" :data-testid="`daemon-${daemon.id}-timer-repeat`"/>
                          </div>
                          <div v-else-if="daemon.trigger.triggerType === 'DayTimeDirect'">
                            {{$t('trigger.DayTimeDirect')}}
                            <q-input type="time" v-model="daemon.trigger.condition.time" :label="$t('time')" :data-testid="`daemon-${daemon.id}-daytime-time`" />
                          </div>
                          <div v-else-if="daemon.trigger.triggerType === 'DateTimeDirect'">
                            {{$t('trigger.DateTimeDirect')}}
                            <q-input type="date" v-model="daemon.trigger.condition.date" :label="$t('date')" :data-testid="`daemon-${daemon.id}-datetime-date`" />
                            <q-input type="time" v-model="daemon.trigger.condition.time" :label="$t('time')" :data-testid="`daemon-${daemon.id}-datetime-time`" />
                          </div>
                          <div v-else>
                        ...
                        <div class="col-8">
                          <div class="q-ma-md q-pa-md shadow-2">
                            {{$t('execution')}}
                            <q-select v-model="daemon.exec.generator" :options="generatorList" options-dense :label="$t('generatorName')" :data-testid="`daemon-${daemon.id}-exec-generator`" />
                            <q-toggle v-model="daemon.exec.directTrigger" :label="$t('directTrigger')" :data-testid="`daemon-${daemon.id}-exec-direct`"/>
                            <q-input type="textarea" class="q-ma-md" v-model="daemon.exec.templateGeneratePrompt" label="Generator prompt template" outlined :disable="daemon.exec.generator == 'emptyText' || daemon.exec.directTrigger" :data-testid="`daemon-${daemon.id}-exec-template`" />
                            <div>{{$t('outputContextAttr')}}</div>
                            <div class="row">
                            <q-select v-model="daemon.exec.setting.toClass" :options="AsClassList" options-dense :label="$t('outputClass')" class="col-6" :data-testid="`daemon-${daemon.id}-out-class`" />
                              <q-select v-model="daemon.exec.setting.toRole" :options="AsRoleList" options-dense :label="$t('outputRole')" class="col-6" :data-testid="`daemon-${daemon.id}-out-role`" />
                              <q-select v-model="daemon.exec.setting.toContext" :options="AsContextLinesList" options-dense :label="$t('outputContext')" class="col-6" :data-testid="`daemon-${daemon.id}-out-context`" />
                            </div>
                          </div>
                        </div>
                        </div>
                ...
                <q-tab-panel name="websocket">
                  ...
                  <q-toggle v-model="editingSettings!.general.useSocket" :label="$t('useWebSocketCommunication')" data-testid="as-use-socket" />
                  <q-input v-model="editingSettings!.general.remoteServer"
                           :disable="!(editingSettings!.general.useSocket)"
                           :label="$t('remoteServerAddress')"
                           data-testid="as-remote-server" />
                </q-tab-panel>
              </q-tab-panels>
            </template>
