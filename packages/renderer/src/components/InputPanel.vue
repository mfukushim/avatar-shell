...
async function sendMessage() {
  ...
  if (inText) {
    datas.push(AsMessage.makeMessage({from: getUserName(), text: inText},'talk','user'));
  }
  if (fileUpload.value) {
    const arrayBuffer = await fileUpload.value.arrayBuffer();
    datas.push(AsMessage.makeMessage({from: getUserName(), mediaBin: arrayBuffer, mimeType:fileUpload.value.type,},'talk','user'));
  }
  if(mcpResource.value) {
    mcpResource.value.contents.forEach(v => {
      if (v.mimeType === 'text/plain' && v.text) {
        ...
        datas.push(AsMessage.makeMessage({from: getUserName(), mediaBin: uint8Array.buffer, mimeType:'text/plain',},'talk','user'))
      } else if (v.mimeType.startsWith('image')) {
        const encoder = new TextEncoder();
        const uint8Array = encoder.encode(v.blob);
        datas.push(AsMessage.makeMessage({from: getUserName(), mediaBin: uint8Array.buffer, mimeType:v.mimeType,},'talk','user'))
      }
    })
  }
