...
const updateExt = async () => {
  data.value = props.timeline.filter(value => isShow(value.asClass,value.role));
  await nextTick();
...
//        v-if="isShow(item.role)"


const isShow = (asClass:AsClass,role: AsRole) => {
  // if(flag.showAssistant && (asClass === "talk" || asClass === "system" || asClass === 'daemon') && role === 'assistant') return true
  let showFlag = false;
  if(flag.showAll) {
    return true;
  }
  ...
  if (flag.showUser) {
    if(asClass === 'talk' && role === 'user') showFlag = true;
    // if(asClass === 'com' && role === 'user') showFlag = true;
  }
  if(flag.showAssistant) {
    if(asClass === 'talk' && role === 'assistant') showFlag = true;
    // if(asClass === 'com' && role === 'assistant') showFlag = true;
  }
  return showFlag;
  // if(asClass === 'system' && flag.showShowSystem || (role === 'system' && flag.showShowSystem)) {}return true;
  // if(asClass === 'com' && !flag.showOnline) return false; //  todo ユーザ識別入れるか
  // return (asClass !== 'system' && (role === 'assistant' && flag.showAssistant || role === 'user' && flag.showUser || role === 'tools' && flag.showScedule))
};
...
-->
        <div class="row" v-if="showInfo">
          ...
          <div >{{item.asClass}}/{{item.role}}</div>
