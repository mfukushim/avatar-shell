...
export abstract class GeminiBaseGenerator extends LlmBaseGenerator {
  ...
  setCurrentContext(content:AsMessageContent[],asClass:AsClass): Effect.Effect<{task:Option.Option<GeneratorTask>}, Error, DocService | ConfigService> {
    const state = this;
    ...
    return Effect.gen(function* () {
      ...
      if (outText) {
        ...
        out.push(AsOutput.makeOutput(AsMessage.makeMessage({
          ...
          text: outText,
        },'talk',GeminiBaseGenerator.generatorInfo.contextRole),{
          provider: state.genName,
          model: state.model,
          isExternal: false,
        },[addPrevious]))
      ...
      if (outImages) {
        ...
        */
        out.push(AsOutput.makeOutput(AsMessage.makeMessage({
          from: avatarState.Name,
          mediaUrl: mediaUrl,
        },state.geminiSettings?.toClass || 'talk',state.geminiSettings?.toRole || GeminiBaseGenerator.generatorInfo.contextRole),
          {
            provider: state.genName,
            model: state.model,
            isExternal: false,
            ...
            return Effect.gen(function* () {
              ...
              return [{
                ...
                mes: {
                  id: nextId,
                  tick: dayjs().valueOf(),
                  asClass: a2.type === 'text' ? 'system' : 'talk', //  TODO ちょっと姑息だがtextだったらsystemにする。それ以外imageとかはtalkにする 後で一貫性について検討要
                  role: 'tools',
