...
*/
      ...
      const appendContext = yield* state.daemonStates.pipe(Ref.get).pipe(Effect.andThen(Effect.forEach(a => {
        ...
        switch (a.config.trigger.triggerType as ContextTrigger) {
          ...
          case 'IfContextExists':
            ...
            const find = updated.delta.find(value => value.asClass !== 'daemon' && value.asClass === a.config.trigger.condition.asClass && value.role === a.config.trigger.condition.asRole);
  ...
  execDaemon(daemon: DaemonState, context: AsMessage[], triggerMes?: AsMessage) {
    ...
    return Effect.gen(function* () {
      ...
      //  出力用にroleとtextは再加工する
      toLlm = out.filter(a => (a.role === 'assistant' || a.role === 'tools')).map(a => {
        return {
          ...a,
          asClass: daemon.config.exec.setting?.toClass || 'daemon',
          role: daemon.config.exec.setting?.toRole || 'system',  //  通常はuserとしてcontextに追加する
          content: {
            ...a.content,
          },
        } as AsMessage;
  ...
  */
  ...
  execGenerator(gen: ContextGenerator, message: AsMessage[], context: AsMessage[] = []) {
    ...
    return Effect.gen(function* () {
      ...
      const {task,} = yield* gen.setCurrentContext(message.map(value => value.content),message[0].asClass);
  ...
  execGeneratorToContext(gen: ContextGenerator, modUserMessage: AsMessage[]) {
    ...
    return Effect.gen(function* () {
      ...
      yield* gen.setPreviousContext(context);
      const {task,} = yield* gen.setCurrentContext(modUserMessage.map(value => value.content),modUserMessage[0].asClass);
