...
export abstract class ClaudeBaseGenerator extends LlmBaseGenerator {
  ...
  setCurrentContext(content: AsMessageContent[],asClass:AsClass): Effect.Effect<{
    ...
    return Effect.gen(function* () {
      ...
      if (outText) {
        ...
        out.push(AsOutput.makeOutput(AsMessage.makeMessage({
            ...
            text: outText,
          }, state.claudeSettings?.toClass || 'talk', state.claudeSettings?.toRole || ClaudeBaseGenerator.generatorInfo.contextRole),
          {
            provider: state.genName,
            model: state.model,
            isExternal: false,
