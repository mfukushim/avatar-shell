...
> MCP Apps are now provisionally supported.  
> Simple MCP Apps will work. Accordingly, support for the traditional MCP-UI has been removed. (Implementation uses MCP-UI AppRenderer.)  
> We plan to unify everything into MCP Apps in the future.  
...
As the official MCP specifications are finalized, we plan to unify the UI processing portion into MCP Apps.  
Currently, simple MCP Apps can be run using the MCP-UI AppRenderer.  

We have confirmed that some of the samples at https://github.com/modelcontextprotocol/ext-apps work.  



The following MCP Apps servers are optimized for Avatar-Shell:  

- reversi-MCP-Apps https://github.com/mfukushim/reversi-mcp-apps  
